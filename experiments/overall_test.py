...
save_dir = opts.out_path + f'test/{opts.config}/'
...
# attributes = {attr: random.choice([-1,1]) for attr in local_attr_dict.keys()}
# print(attributes)

scales = [1, 1.5, 2, 3]
all_losses = [{s: {"loss": [], "loss_pb": [], "loss_reg": [], "loss_recon": []} for s in scales + ["Single"]} for _ in range(2)]
print(all_losses)
...
with torch.no_grad():
    ...
    for s in track(scales, "Evaluating different coeff scaling values"):
        ...
        for i in [0,1]:
            w_0 = np.load(f'./data/teaser/latent_code_0000{i+1}.npy')
            w_0 = torch.tensor(w_0).to(DEVICE)
            x_0, _ = trainer.StyleGAN([w_0], input_is_latent=True, randomize_noise=False) 
            img_l = [x_0]

            coeffs = torch.zeros((1,len(attrs))).to(DEVICE)
            for _, (attr, coeff) in enumerate(attributes.items()):
                ...
                all_losses[i][s]["loss"].append(loss.item())
                all_losses[i][s]["loss_pb"].append(loss_pb.item())
                all_losses[i][s]["loss_reg"].append(loss_reg.item())
                all_losses[i][s]["loss_recon"].append(loss_recon.item())
            ...
            utils.save_image(clip_img(out), save_dir + f"test_multi_{i}_{s}.png")
    ...
    for i in [0,1]:
        ...
        coeffs = torch.zeros((1,len(attrs))).to(DEVICE)
        for _, (attr, coeff) in track(enumerate(attributes.items()), "Evaluating Single Model"):
            attr_idx = local_attr_dict[attr]
            coeffs[0][attr_idx] = coeff
            ...
            all_losses[i]["Single"]["loss"].append(loss.item())
            all_losses[i]["Single"]["loss_pb"].append(loss_pb.item())
            all_losses[i]["Single"]["loss_reg"].append(loss_reg.item())
            all_losses[i]["Single"]["loss_recon"].append(loss_recon.item())
        ...
        out = torch.cat(img_l, dim=3)
        utils.save_image(clip_img(out), save_dir + f"test_multi_{i}_Single.png")

for i in range(2):
    for l in ["loss", "loss_pb", "loss_reg", "loss_recon"]:
        plt.figure(figsize=(12,6))
        for s in all_losses[i].keys():
            if s == "Single":
                label = "SingleAttr"
            else:
                label = f"MultiAttr: {s}"
            plt.plot(range(1,len(attributes)+1), all_losses[i][s][l], label=label)
        ...
        plt.savefig(f"{save_dir}overall_{i}_{l}.png", bbox_inches="tight")
        plt.clf()
