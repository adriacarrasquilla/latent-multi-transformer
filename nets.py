...
#----------------------------------------------------------------------------
# Second Modification of the Mapping network to accept multilabel training

class F_mapping_multi2(nn.Module):
    def __init__(
        self,
        dlatent_size            = 512,              # Transformed latent (W) dimensionality.
        mapping_layers          = 18,               # Number of mapping layers.
...
class F_mapping_multi3(nn.Module):
    def __init__(
        self,
        dlatent_size            = 512,              # Transformed latent (W) dimensionality.
        mapping_layers          = 18,               # Number of mapping layers.
        ...
        **_kwargs):                                 # Ignore unrecognized keyword args.
        ...
        self.dense = nn.ModuleList()
        for _ in range(mapping_layers):
            self.dense.append(Dense_layer(mapping_fmaps+n_attributes, mapping_fmaps, lrmul=mapping_lrmul))
    ...
    def forward(self, latents_in, coeffs):
        ...
        # Mapping layers.
        for layer_idx in range(self.mapping_layers):
            out.append(apply_bias_act(self.dense[layer_idx](torch.cat((x[layer_idx], coeffs), dim=1)), act='linear'))
